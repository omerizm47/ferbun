 ...
 * threshold — a new level, a new streak tier, or an earned achievement badge.
 * Reuses the Newroz-sun medal language (ray-burst behind a gradient emblem +
 * kilim accent) so the milestone feels crafted and on-brand. Leads with the
 * attested Kurmanji "Pîroz be!".
 */
export default function CelebrationOverlay({ celebration, onDismiss }: Props) {
  ...
  if (celebration.kind === 'level') {
    overline = 'LEVEL UP';
    detail = `You reached Level ${celebration.level}`;
  } else if (celebration.kind === 'streak') {
    overline = 'NEW STREAK TIER';
    detail = celebration.tier.label;
  } else if (celebration.kind === 'badge') {
    const badge = ALL_BADGES.find((b) => b.id === celebration.badgeId);
    overline = lang === 'tr' ? 'YENİ BAŞARIM' : 'NEW ACHIEVEMENT';
  ...
  const renderMedalContent = () => {
    ...
    switch (badge.motif) {
      ...
      default:
        return <Ionicons name={badge.icon as any} size={size} color={color} />;
    }
  };
